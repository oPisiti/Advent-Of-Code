...
std::string applyMask(const std::string& data, const std::string& mask){
    ...
    for(int i = 0; i < mask.size(); i++){
        if      (mask[i] == 'X'){sDataBin[i] = 'X';}
        else if (mask[i] == '1'){sDataBin[i] = '1';}
        else                    {sDataBin[i] = sDataBin[i];}
    }

    return sDataBin;
}

void writeAllToMemory(std::map<std::string, std::string>& mem, std::string sData, int iStartInd = 0){
    std::string sModifiedDataA = sData, sModifiedDataB = sData;
    for(int i = iStartInd; i < sData.size(); i++){
        if(sData[i] == 'X'){
            std::replace();
            writeAllToMemory(mem, sData, i+1)
            writeAllToMemory(mem, sData, i+1)
        }
    }
}
        ...
        if(line.substr(0, 4) == "mask"){
            ...
            while (std::regex_search(searchStart, line.cend(), m, regex)) {
                if(!i%2){ index = m.str(0);}
                    ...
                    dataToWrite = applyMask(m.str(0), mask);
                    writeAllToMemory(mem, dataToWrite);
