...
class Cave():
    ...
    def drop_sand(self, sand_pos: Point2d) :
        ...
        while True:
            ...
            # Reached floor
            abyss_msg = "Full of sand"
            if self.map[sand_pos.i+1][sand_pos.j - self.j_starts_at] == "O": raise IndexError(abyss_msg)
    ...
    def add_column(self, where: str) -> None:
        index = int()
        size_map = len(self.map)
        ...
        if   where == "left":
            ...
            end_a = a
            end_b = b 
            print(f'Adding RIGHT')
        else: raise RuntimeError(f"Adding column '{where}' unsupported'")

        for i in range(size_map):
            if i < (size_map-1): self.map[i] = start_a + self.map[i] + end_a
            else:                self.map[i] = start_b + self.map[i] + end_b
