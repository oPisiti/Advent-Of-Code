...
class Global:
    ...
    high_pulse_count = 0
    queue = Queue()
    cycles_count = {k: None for k in ("sr", "sn", "vq", "rf")}
...
class Broadcaster:
    ...
    def run(self, data: dict, modules: dict, i: int) -> None:
...
class Conjunction:
    ...
    def run(self, data: dict, modules: dict, i: int) -> None:
        ...
        # Part 02
        if self.name in Global.cycles_count.keys():
            if pulse_value == Pulse.HIGH and Global.cycles_count[self.name] is None:
                Global.cycles_count[self.name] = i
                pass
...
def pulse_propagation():
    ...
    while True:
        ...
        while not Global.queue.empty():
            ...
            modules[op_info["to"]].run(op_info, modules, i)

            # Found all multiples
            if None not in Global.cycles_count.values():
                return lcm(*[a for a in Global.cycles_count.values()])
