...
def heat_loss():
    ...
    Global.queue = [Node(Index2D(1, 0), Global.maze[0][0], Index2D(0, 0), 1),
                    Node(Index2D(0, 1), Global.maze[0][0], Index2D(0, 0), 1)]

    Global.nodes = [[Node(Index2D(i, j), math.inf, None, 1) for i in range(len(data[0]))] for j in range(len(data))]
    # Global.heat_loss = [[math.inf for _ in range(len(data[0]))] for l in range(len(data))]
    del data

    # Global.heat_loss[0][0] = 0
    # Main loop
    while Global.queue:
...
def march_crucible(node: Node) -> None:
    ...
    # There is a better path to this
    if next_score > Global.nodes[node.pos.i][node.pos.j].score: return
